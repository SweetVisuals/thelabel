...
import { X, Calendar, Clock, AlertCircle, CheckCircle, Loader2, ChevronRight, Image as ImageIcon, Film, Send, Settings, Layers, CalendarDays, User, Timer, Play } from 'lucide-react';
...
import { addHours, addMinutes, format, startOfMonth, endOfMonth, eachDayOfInterval, isSameMonth, isSameDay, setHours, setMinutes } from 'date-fns';
  ...
  const previewSchedule = useMemo(() => {
    ...
    if (postingStrategy === 'first-now') {
      ...
      for (let i = 0; i < slideshows.length; i++) {
        ...
        let batchStartTime = addMinutes(startTime, batchIndex * (batchIntervalHours * 60));
        let postTime = addMinutes(batchStartTime, indexInBatch * postIntervalMinutes);
        postTime = applyScheduleConstraints(postTime);

        let batchInfo = null;
        if (indexInBatch === 0) {
          batchInfo = { number: batchIndex + 1, time: batchStartTime };
        }
