  ...
  const handleFolderDrop = async (e: React.DragEvent, targetFolderId: string) => {
    ...
    try {
      ...
      if (data.type === 'file') {
        ...
        // Slideshows don't support folders yet in the DB schema provided in types, 
        // but let's assume we might want to add that later. 
        // Based on types/index.ts, SlideshowMetadata has folder_id.
        // So we can implement this if slideshowService supports it.
        // For now, let's just handle images as per request "drag and drop images into folders"
        // But let's check if we can move slideshows too.
        // The user request specifically said "images", but consistency is good.
        // Let's stick to images first to be safe, or try to update slideshows if possible.
        // Looking at slideshowService (not fully visible but assumed), let's try to update slideshows too if we can.
        // Actually, let's check if we can simply update the folder_id for slideshows.
        // Since I don't have the full slideshowService code for moving, I'll stick to images for now 
        // unless I see a moveSlideshowsToFolder method.
        // I'll stick to images for now to ensure reliability.

        // Wait, looking at types, SlideshowMetadata DOES have folder_id.
        // Let's try to move slideshows by updating them one by one if no batch method exists.
        // For now, I will implement image moving as requested.
      }
