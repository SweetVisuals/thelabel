...
export interface BulkCreateFromTemplateModalProps {
  ...
  onBulkCreate: (
    ...
    imagesToUse: UploadedImage[],
    onProgress?: (progress: number, current: number, total: number) => void
  ) => Promise<BulkTemplateCreationResult>;
...
export const BulkCreateFromTemplateModal: React.FC<BulkCreateFromTemplateModalProps> = ({
  ...
  const [progress, setProgress] = useState<{ percent: number; current: number; total: number }>({ percent: 0, current: 0, total: 0 });
  ...
  const targetImages = React.useMemo(() => {
    if (selectedImageIds.length > 0) {
      return uploadedImages.filter(img => selectedImageIds.includes(img.id));
    }
    return uploadedImages;
  }, [uploadedImages, selectedImageIds]);

  // Update preview when template, images, randomize setting, or slides per slideshow changes
  useEffect(() => {
    if (selectedTemplateData && targetImages.length > 0) {
  ...
  }, [selectedTemplateData, targetImages, randomizeImages, slidesPerSlideshow]);
        ...
        <div className="p-6 border-b border-border">
          ...
          <p className="text-sm text-muted-foreground mt-1">
            {selectedImageIds.length > 0
              ? `Create multiple slideshows from a single template using your ${selectedImageIds.length} selected images`
        ...
        <div className="p-6 space-y-6 max-h-[calc(90vh-140px)] overflow-y-auto">
          {templates.length === 0 ? (
            ...
            <>
              ...
              {/* Options */}
              <div className={cn("space-y-4", isCreating && "opacity-50 pointer-events-none")}>
                <div>
              ...
              {isCreating && (
                <div className="space-y-2">
                  <div className="flex justify-between text-sm">
                    <span className="font-medium">Creating Slideshows...</span>
                    <span className="text-muted-foreground">{progress.percent}% ({progress.current}/{progress.total})</span>
                  </div>
